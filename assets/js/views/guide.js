...
const SALONONE_TASKS = [
    ...
    { what: '会計（支払い方法を正しく選ぶ・物販は物販として登録）', why: '売上・客単価・入金突合・出納帳の現金売上・物販インセンティブ' },
...
const TOOL_TASKS = [
    ...
    { head: '入れる（SalonOneにない数字だけ）', items: ['日報: 次回予約（新規/既存）・ブログ・SNS・★5口コミ', '出納帳: 経費・預け入れ・釣り銭の出し入れ、レジの実査と日次締め（現金売上はSalonOneから自動）', '入金突合: カード/QRの端末集計（現金は出納帳の締めから自動）', '目標・基本給、HPBなどAPIにない広告費'] },
...
const ROUTINES = {
    ...
    store: {
        label: '店長',
        daily: ['現金で払った経費・預け入れ・釣り銭の補充はその場で出納帳に記録', '閉店後にレジの現金を数えて出納帳を締める（過不足があれば理由を入力）', 'カード/QRは入金突合に端末の日計を入力', 'ホームで日報の未入力者を確認して声かけ', 'SalonOneの会計未処理・担当者未設定をなくす'],
        weekly: ['スタッフ別の次回予約率・客単価・キャンセル率を確認', '着地予測と目標の差を見て打ち手を決める'],
        monthly: ['月末までに翌月の目標を設定', '申請締切後にシフトを自動分配 → 調整 → 承認', '月末に今月の未突合・出納帳の未締めをゼロにし、出納帳CSVを経理へ'],
        numbers: ['店舗の目標進捗・着地予測', '日報の入力率', '次回予約率（新規・既存）', '新規の入会率・キャンセル率', '現金過不足（出納帳）'],
    },
    manager: {
        label: 'マネージャー',
        daily: ['ホームで全店舗の未突合・出納帳の未締め・日報未入力を確認'],
    ...
    admin: {
        label: 'オーナー',
        daily: ['ホームの「やること」（未突合・出納帳の未締めや締め後の変更・設定の不足）を確認'],
        weekly: ['全店舗の着地予測と目標の差', 'SalonOneで確認すること（入力漏れ）が残っていないか', '出納帳の過不足と理由・締めの取消を操作ログで確認'],
