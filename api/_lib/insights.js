...
const { fetchAllPages } = require('./salonone');
...
const CANCEL_TIME_KEYS = ['canceled_at', 'cancelled_at'];
const DONE_TIME_KEYS = ['checked_out_at', 'checkout_at', 'paid_at', 'accounted_at', 'completed_at', 'settled_at', 'visited_at'];
...
function classify(row) {
    ...
    if (RE_NOSHOW.test(status) || row.is_no_show === true || row.no_show === true) return 'no_show';
...
async function appointmentInsights({ from, to, shopId = null, today }) {
    ...
    const kindCounts = { done: 0, canceled: 0, no_show: 0, open: 0 };
    for (const a of appts) kindCounts[a.kind]++;
    ...
    for (const v of appts) {
        if (v.kind !== 'done' || v.date < from || v.date > to) continue;
        ...
        let isNew = v.isNew;
        if (isNew === null && firstVisit && v.customer) isNew = firstVisit.get(v.customer) === v.date;
    ...
    for (const a of appts) {
        if (a.kind !== 'open' || a.date < from || a.date > to || a.date >= today) continue;
        unsettled.count++;
...
module.exports = { appointmentInsights, _internal: { classify, newFlag, parseUtc, jstDate } };
