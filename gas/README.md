...
> **⚠️ v2 更新のお知らせ（要・再デプロイ）**
> ダッシュボード v2（広告費ROI・インセンティブ月締め確定）を利用するには、最新の `Code.gs` を貼り直して **「デプロイを管理」→ 鉛筆アイコン → バージョン「新バージョン」→ デプロイ** を実行してください（URLは変わりません）。
> 追加された設定キー: `ad_costs`（媒体広告費）, `monthly_close`（月締め確定スナップショット）— いずれも「ダッシュボード設定」シートに保存されます。
> ※再デプロイ前でも該当2機能以外はすべて動作します（広告費・月締めはローカル保存のみになります）。
