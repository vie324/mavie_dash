...
function parseCustomerSheetOptimized(sheet, store) {
  ...
  if (!cols) {
    ...
    cols = {
      ...
      eyebrowDesignImage: findCol(['眉毛メニュー】ご希望のデザインイメージ', '眉毛】デザインイメージ']),
      ...
      lashDesignImage: findCol(['まつ毛メニュー】ご希望のデザインイメージ', 'まつ毛】デザインイメージ']),
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    if (dateStr || name) {
      result.push({
        ...
        eyebrowDesignImage: getStr(row, cols.eyebrowDesignImage),
        ...
        lashDesignImage: getStr(row, cols.lashDesignImage),
